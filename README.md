...
- CI(지속적 통합)는 코드를 push할 때마다 자동으로 품질검사를 수행하는 프로세스
...
- Github Actions 워크플로우는 **YAML 파일**로 정의한다.
- Github는 `.github/workflows/` 경로의 YAML 파일을 자동으로 인식한다.
```
...
jobs:                              # 무엇을 실행할 것인가?
  job-name:                        # 작업 이름
    runs-on: ubuntu-latest         # 어디서 실행하는가? (가상머신)
    steps:                         # 실행 단계들 (순차 실행)
